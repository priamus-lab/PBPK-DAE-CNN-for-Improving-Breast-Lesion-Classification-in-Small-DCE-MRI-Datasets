...
class CSV_DatasetFolder(Dataset):
  ...
  def __getitem__(self, index: int):
    ...
    sha_image = sio.loadmat(self.imagePath + '/' + self.list_classes[target] + '/' + sha_path)['new_image']
    assert np.max(sha_image)<=1
    assert np.min(sha_image)>=0
    
    warp_image = sio.loadmat(self.imagePath + '/' + self.list_classes[target] + '/' + warp_path)['new_image']
...
def step4_Decoder_onFake(model_conv, encoders,decoders, 
                         ...
                         train_loss_totale_fake, totalLoss_pbpk_fake, epoch):
...
def train_iteration(epochs, opt, model_conv, encoders, decoders, trainSet_real,trainSet_csv,
                    optimizer_conv, updator_encoders, updator_decoders, optimizer_conv_decoders,
                    loader_opts,
  ...
  return model_conv, encoders, decoders
...
def train_loop_validation(model_conv, encoders, decoders, dim_to_resize,
                          trainSet_real, trainSet_csv, valSet_real, valSet_csv, testSet_real,
                          optimizer_conv, updator_encoders, updator_decoders, optimizer_conv_decoders,
  ...
  for epochs in range(start, num_epoch + 1):
    ...
    encoders.train()
    decoders.train()
    model_conv, encoders, decoders = train_iteration(epochs, opt, model_conv, encoders, decoders, 
                                                     trainSet_real,trainSet_csv,
    ...
    if to_validate and (epochs == 1 or (modelLoss_epoch_val<best_loss)):  #or (mean_accuracy_val >= best_acc_m) or (modelLoss_epoch_val<best_loss)):   
