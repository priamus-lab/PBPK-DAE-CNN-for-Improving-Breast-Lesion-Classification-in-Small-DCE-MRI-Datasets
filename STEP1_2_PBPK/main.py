...
parser.add_argument('--lr', type=float, default=0.0000002, help='learning rate, default=0.0002')
...
pathResult_train = basePath+'Results/STEP_PBPK_FINAL_Version/DAE_CNN_STEP1_2_' + net_name + '/TRAIN_LOOP/Fold'+str(fold) + '/' #ci mettiamo i pesi nel corso degli addestramenti->ci sono i pesi migliori e quelli di ogni epoca
pathResult_final = basePath+'Results/STEP_PBPK_FINAL_Version/DAE_CNN_STEP1_2_' + net_name + '/FINAL_LOOP/Fold'+str(fold) + '/' #ci mettiamo i pesi nel riaddestramento -> pesi di ogni epoca del riaddestramento
...
print('   [TRAIN CSV] Benign: %d Malignant %d ' %(len(Trainset_csv[0]),len(Trainset_csv[1])))
completeTrainSet_csv = BalanceConcatDataset(Trainset_csv)
print('   [TRAIN CSV] Balance - - - Benign: %d Malignant %d ' %(len(Trainset_csv[0]),len(Trainset_csv[1])))

Val_csv = []
dataset = CSV_DatasetFolder(root = pathTocsv, csv_path = 'df_benign.csv',
...
train_loop_validation(model_conv, encoders, decoders, dim_to_resize,
                      completeTrainSet, completeTrainSet_csv, dataset_real_validation, Val_csv_final, dataset_real_test,
